...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args or args[0] in ("-h", "--help"):
        print("usage: strip.py {" + ",".join(SUBCOMMANDS) + "} <docx> [flags…]\n"
              "每个子命令的参数与原独立脚本逐字一致：strip.py <sub> --help 查看。")
        return 0 if args else 2
    sub, rest = args[0], args[1:]
    fn = SUBCOMMANDS.get(sub)
    if fn is None:
        print(f"[strip] unknown subcommand: {sub!r}; choices={list(SUBCOMMANDS)}",
              file=sys.stderr)
        return 2
    saved = sys.argv[:]
