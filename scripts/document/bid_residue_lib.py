...
# 中文层级序号的合法形态：行首「1）」「（1）」「1)」——院里钦定的五/六级标题写法
# （2026-08-30 桐乡实证：60 条五级标题补编号后被整片误判成删句残渣）。
# 只豁免**行首**这一个孤立右括号，句中的「，此处）」照旧算残渣。
LEAD_ORDINAL = re.compile(r"^\s*(（\d+）|\d+[）\)])")


def paren_unbalanced(text):
    """段内全角括号失衡（删句留尾巴的典型形态,如「本章不复写）：」）。"""
    m = LEAD_ORDINAL.match(text)
    if m:
        text = text[m.end():]
    d = 0
    for ch in text:
...
CAT_ADVICE = {
    ...
    3: "删脚手架块/照抄裸句；评分元语言删去或改写为对业主要求的直接响应（如「本项目要求…」），不写招标/采购程序用语；去直呼评委；项目禁词见 banned_terms，如「招标文件要求」→「本项目要求」",
...
XREF_PATS = [
    ...
    # 无编号位置指针（2026-08-31 补）：旧的四条**每条都要求有数字**，于是
    # 「见本节「其他技术支撑工作」」「详见前文」这类整类漏检 —— 去掉编号不等于去掉耦合，
    # 合稿人删调章节后这种指针一样断链。实证：当天我把「本章 5.8」改成「见本节「X」」，
    # deref --check 报 exit 0 放行。
    # 豁免「见下表/上表/下图/上图」：那是 deref 自己的修复终态（邻近表图→「下表」），不是耦合。
    re.compile(r"(?:详见|参见|另见|见)\s*(?:本章|本节|前文|上文|后文|下文|前述)"),
    re.compile(r"(?:详见|参见|另见|见)\s*本(?:章|节|文件)?\s*[「『《]"),
    re.compile(r"如(?:前|上)所述|上述(?:章节|各节|各章)"),
]
