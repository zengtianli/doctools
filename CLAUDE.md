...
| 脚本 | 干什么 | 怎么跑 |
|---|---|---|
| `scripts/document/word_table_format.command` | Mac Word 表格去底纹、居中、0.5 磅黑色实线；图名/表名套已有 ZDWP图名 样式（复用 `lib/caption_re.py`）；自动备份，Word 文档接口写入 | Word 打开目标 `.docx` 后双击；或 `word_table_format.command [当前文档完整路径]`；`--help` 只显示帮助 |
